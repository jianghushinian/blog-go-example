...
func Example() {
	...
	pp(nil)
	v, err := Get(person, "name", "first")
	fmt.Printf("First name: %v, error: %v\n", v, err)

	// Change first name:
	pp(Set(person, "Alice", "name", "first"))

	// Change complete name from map to a single string:
	pp(Set(person, "Alice Archer", "name"))

	// Print and increment age:
	age, err := GetInt(person, "age")
	fmt.Printf("Age: %v, error: %v\n", age, err)
	pp(Set(person, age+1, "age"))

	// Change a fruits slice element:
	pp(Set(person, "lemon", "fruits", 1))

	// Add a new fruit:
	pp(Append(person, "melon", "fruits"))
...
func Example_jsonEdit() {
	...
	user, err := Get(v, "login", "user")
	fmt.Printf("User:        %-6s, error: %v\n", user, err)

	password, err := Get(v, "login", "password")
	fmt.Printf("Password:    %-6s, error: %v\n", password, err)

	// Edit (mask out) password:
	if err = Set(v, "xxx", "login", "password"); err != nil {
...
func ExampleAppend() {
	...
	printMap(Append(m, 4, "a"))
	printMap(Append(m, 9, "a", 2))
	printMap(Append(m, 1, "x"))
...
func ExampleAppendMore() {
	...
	err := AppendMore(m, []interface{}{3, 4, 5}, "ints")
	fmt.Println(m, err)
